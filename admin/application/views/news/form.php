...
<div class="row" id="toni-breadcrumb">
    <div class="col-6">
        <h4 class="text-dark mt-2">News</h4>
    </div>
    <div class="col-6">
  ...
  <div class="row" id="toni-content">
    <div class="col-md-12">
      <form action="<?php echo (site_url('news/simpan')) ?>" method="post" id="form" enctype="multipart/form-data">
        <div class="row">
          <div class="col-md-12">
...
?>
                  ...
                  <input type="hidden" name="idnews" id="idnews">
                  ...
                  <div class="form-group row required">
                    ...
                    <div class="col-md-9 row">
                      <div class="col-md-12">
                        <input type="text" name="judulnews" id="judulnews" class="form-control" placeholder="Judul">
                      </div>
                      <div class="col-md-12">
                            <label for="">Link Url: <a href="" id="judulnewsseo" target="_blank"></a></label>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="form-group row required">
                    <label for="" class="col-md-12 col-form-label">Isi News</label>
                    <div class="col-md-12">
                      <textarea name="isinews" id="isinews" class="form-control" rows="10" placeholder="Isi news"></textarea>
                    </div>
                  </div>
              ...
              <div class="card-footer">
                ...
                <a href="<?php echo (site_url('news')) ?>" class="btn btn-default float-right mr-1 ml-1"><i class="fa fa-chevron-circle-left"></i> Kembali</a>
              </div>
            </div> <!-- /.card -->
          </div> <!-- /.col -->
        </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    if ( idnews != "" ) {
          $.ajax({
              type        : 'POST',
              url         : '<?php echo site_url("news/get_edit_data") ?>',
              data        : {idnews: idnews},
              dataType    : 'json',
              encode      : true
          })
          .done(function(result) {
            $("#idnews").val(result.idnews);
            $("#judulnews").val(result.judulnews);
            $("#judulnewsseo").html(result.judulnewsseo);
            $("#isinews").val(result.isinews);
            ...
            if ( result.gambarsampul != '' && result.gambarsampul != null ) {
                $("#output1").attr("src","<?php echo (base_url('../uploads/news/')) ?>" + result.gambarsampul);
            }else{
                $("#output1").attr("src","<?php echo (base_url('../images/nofoto.png')) ?>");
            }


            CKEDITOR.replace('isinews' ,{
                filebrowserImageBrowseUrl : '<?php echo base_url('.../uploads/galery'); ?>',
                height : ['400px'],
              });
            CKEDITOR.instances.isinews.setData( result.isinews );

          });


          $("#lbljudul").html("Edit News");
          $("#lblactive").html("Edit");
    ...
    $("#form").bootstrapValidator({
      ...
      fields: {
        judulnews: {
          validators:{
            notEmpty: {
                message: "judulnews tidak boleh kosong"
            },
          }
        },
        isinews: {
          validators:{
            notEmpty: {
                message: "isinews tidak boleh kosong"
            },
          }
        },
        tglinsert: {
