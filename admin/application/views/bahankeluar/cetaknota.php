...
$rowhead = $this->db->query("select * from v_bahankeluar where idbahankeluar='".$idbahankeluar."'")->row();
					...
					<table border="0" cellpadding="0">
							...
							<tr>
								...
								<th width="50%" style="font-size:14px; text-align:left;">
									'.tglindonesia($rowhead->tglbahankeluar).'
								</th>
							</tr>
							<tr>
								<th width="40%" style="font-size:14px; text-align:left;">
								...
								<th width="50%" style="font-size:14px; text-align:left;">
									'.hariini(date('Y-m-d', strtotime($rowhead->tglbahankeluar))).'
								</th>
							</tr>
							<tr>
								<th width="40%" style="font-size:14px; text-align:left;">
								...
								<th width="50%" style="font-size:14px; text-align:left;">
									'.date('H:i:s', strtotime($rowhead->tglbahankeluar)) .'
								</th>
							</tr>
							<tr>
								<th width="40%" style="font-size:14px; text-align:left;">
...
$table .= '
	<div style="text-align: center; font-size: 24px; background-color: #ACACAC;">
		<span style="padding-top: 10px;">BARANG KELUAR REMAHAN</span>
	</div>
';
...
$rsdetail = $this->db->query("select * from v_bahankeluardetail where idbahankeluar='".$idbahankeluar."'");
if ($rsdetail->num_rows()>0) {
	...
	foreach ($rsdetail->result() as $rowdetail) {
		...
		$table .= '<tr style="font-size:12px;">
						...
						<td width="15%" style="text-align:center;">'.$rowdetail->idbahankeluar.'</td>
					...
					';
		$subtotalberatbahan += $rowdetail->beratbahankeluar;
	}
